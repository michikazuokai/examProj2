...
- 対象：subjectNo=1010401, fsyear=2025（必要に応じて読み替え）
...
```
python manage.py shell -c "from exam2.models import Subject; s=Subject.objects.get(subjectNo='1010401',fsyear=2025); print(s.id,s.subjectNo,s.fsyear,s.term,s.nenji,s.name)"
```
...
```
python manage.py clear_subject_data 1010401
```
...
Phase3運用では **Subjectは残し、Exam以下が0** になっているのが正しいです。
...
> 補足：あなたの実行結果で「Subjectが1件残る」は **正常**です（Phase3の設計どおり）。
...
```
python manage.py load_student_exam_version 1010401 --fsyear 2025 --dry-run
```
...
```
python manage.py load_student_exam 1010401 --fsyear 2025 --dry-run
```
...
```
python manage.py load_student_exam 1010401 --fsyear 2025 --batch-size 2000
```
...
```
python manage.py load_exam_adjust 1010401 --fsyear 2025 --dry-run
```
...
```
python manage.py load_exam_adjust 1010401 --fsyear 2025 --batch-size 2000
```
...
# **⚠️ 重要メモ（clear_subject_data の仕様固定）**





Phase3運用では、**Subjectは残して Exam以下を消す**のが安全です。
...
→ load_student_exam_version.py 側で str(fsyear) と int 両方対応しているなら通常は通ります。
...
- subjectNo だけで消していて、fsyear違い の Subject が混在している可能性
